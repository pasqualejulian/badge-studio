...
export class BadgeEngine {
  ...
  dispose(){this.recording?.abort();this.playback.stop();this.maps.dispose();this.disposed=true;this.renderer.domElement.removeEventListener('wheel',this.cancelCamera);this.renderer.domElement.removeEventListener('pointerdown',this.pointerDown);this.renderer.domElement.removeEventListener('pointerup',this.pointerUp);this.renderer.domElement.removeEventListener('pointercancel',this.pointerCancel);this.select(null);this.outlineMaterial.dispose();cancelAnimationFrame(this.raf);this.resize.disconnect();this.controls.dispose();this.badge.traverse(o=>{if(o instanceof THREE.Mesh)o.geometry.dispose();});this.clearRelief();this.metal.dispose();this.frameMaterial.dispose();this.enamel.dispose();this.texture.dispose();this.env?.dispose();this.surfaces.dispose();this.motion.dispose();this.renderer.dispose();this.renderer.domElement.remove();}
}
