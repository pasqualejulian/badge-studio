import {withSelectionFocus} from './selection-focus';
...
export class BadgeEngine {
  ...
  private pointerUp=(e:PointerEvent)=>{if(this.playback.active||this.exporting)return;const start=this.down;const single=this.pointers.size===1;this.pointers.delete(e.pointerId);this.down=null;if(!start||!single||!isSelectionGesture(start,{x:e.clientX,y:e.clientY}))return;const picked=pickBadge(this.camera,this.badge.children.filter(o=>!this.isolateLayer||this.selected?.kind!=='layer'||o.userData.pick?.kind!=='layer'||o.userData.pick?.id===this.selected.id),this.renderer.domElement.getBoundingClientRect(),e.clientX,e.clientY);this.select(picked);};
  private pointerCancel=()=>{this.down=null;this.pointers.clear();};
  private focusAmount=0; private isolateLayer=false;
  setIsolateLayer(value:boolean){this.isolateLayer=value;}
  ...
  dispose(){this.engraving.dispose();this.recording?.abort();this.playback.stop();this.maps.dispose();this.disposed=true;this.renderer.domElement.removeEventListener('wheel',this.cancelCamera);this.renderer.domElement.removeEventListener('pointerdown',this.pointerDown);this.renderer.domElement.removeEventListener('pointerup',this.pointerUp);this.renderer.domElement.removeEventListener('pointercancel',this.pointerCancel);this.select(null);this.outlineMaterial.dispose();cancelAnimationFrame(this.raf);this.resize.disconnect();this.controls.dispose();this.badge.traverse(o=>{if(o instanceof THREE.Mesh)o.geometry.dispose();});this.clearRelief();this.metal.dispose();this.frameMaterial.dispose();this.enamel.dispose();this.texture.dispose();this.env?.dispose();this.surfaces.dispose();this.motion.dispose();this.renderer.dispose();this.renderer.domElement.remove();}
}
