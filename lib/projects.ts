...
export interface BadgeProject{format:'badge-studio';version:1;collectionId?:string|null;id:string;name:string;system:string;kind:'piece'|'template';sourceTemplate:string|null;updatedAt:string;thumbnail:string;scene:SceneSnapshot;motion:MotionOptions;labels:{base:string;design:string}}
...
export function validateProject(input:unknown):BadgeProject{
 ...
 return {format:'badge-studio',version:1,collectionId:p.collectionId==null?null:str(p.collectionId,100),id:str(p.id,100),name:str(p.name,100).trim()||'Sin título',system:str(p.system,100),kind:one(p.kind,['piece','template']),sourceTemplate:p.sourceTemplate===null?null:str(p.sourceTemplate,100),updatedAt:str(p.updatedAt,50),thumbnail:image(p.thumbnail),scene:{engraving:validateEngraving(s.engraving),basePoints:points,baseSVG:s.baseSVG===null?null:str(s.baseSVG,512000),designSVG:s.designSVG===null?null:str(s.designSVG,512000),raster:s.raster===null?null:image(s.raster),settings:clean,layers,maps,camera:{position,target}},motion:{preset:one(mo.preset,['turntable','sway']),duration:num(mo.duration,2,15),loop:bool(mo.loop),background:color(mo.background)},labels:{base:str(labels.base),design:str(labels.design)}};
}
...
export {projectStore,collectionStore,readLibrary} from './project-storage.ts';
