...
export interface SceneSnapshot{basePoints:number[][];baseSVG:string|null;designSVG:string|null;raster:string|null;settings:BadgeSettings;layers:ReliefLayer[];maps:Record<string,{settings:MapSettings;images:Partial<Record<MapKind,string>>}>;camera:{position:number[];target:number[]}}
